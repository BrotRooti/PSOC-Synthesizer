...
/*******************************************************************************
* Function Name: WaveDAC1_DacClk_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_Start(void) 
{
    /* Set the bit to enable the clock. */
    WaveDAC1_DacClk_CLKEN |= WaveDAC1_DacClk_CLKEN_MASK;
	WaveDAC1_DacClk_CLKSTBY |= WaveDAC1_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: WaveDAC1_DacClk_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    WaveDAC1_DacClk_CLKEN &= (uint8)(~WaveDAC1_DacClk_CLKEN_MASK);
	WaveDAC1_DacClk_CLKSTBY &= (uint8)(~WaveDAC1_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: WaveDAC1_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_StopBlock(void) 
{
    if ((WaveDAC1_DacClk_CLKEN & WaveDAC1_DacClk_CLKEN_MASK) != 0u)
    {
...
#if defined(WaveDAC1_DacClk__CFG3)
        CLK_DIST_AMASK = WaveDAC1_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = WaveDAC1_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* WaveDAC1_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(WaveDAC1_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(WaveDAC1_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: WaveDAC1_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        WaveDAC1_DacClk_CLKSTBY &= (uint8)(~WaveDAC1_DacClk_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: WaveDAC1_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = WaveDAC1_DacClk_GetSourceRegister();
    uint16 oldDivider = WaveDAC1_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = WaveDAC1_DacClk_CLKEN & WaveDAC1_DacClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(WaveDAC1_DacClk_DIV_PTR, clkDivider);
                WaveDAC1_DacClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                WaveDAC1_DacClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(WaveDAC1_DacClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((WaveDAC1_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: WaveDAC1_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 WaveDAC1_DacClk_GetDividerRegister(void) 
{
    return CY_GET_REG16(WaveDAC1_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: WaveDAC1_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_SetModeRegister(uint8 modeBitMask) 
{
    WaveDAC1_DacClk_MOD_SRC |= modeBitMask & (uint8)WaveDAC1_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: WaveDAC1_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_ClearModeRegister(uint8 modeBitMask) 
{
    WaveDAC1_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(WaveDAC1_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: WaveDAC1_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 WaveDAC1_DacClk_GetModeRegister(void) 
{
    return WaveDAC1_DacClk_MOD_SRC & (uint8)(WaveDAC1_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: WaveDAC1_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = WaveDAC1_DacClk_GetDividerRegister();
    uint8 oldSrc = WaveDAC1_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        WaveDAC1_DacClk_MOD_SRC |= CYCLK_SSS;
        WaveDAC1_DacClk_MOD_SRC =
            (WaveDAC1_DacClk_MOD_SRC & (uint8)(~WaveDAC1_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: WaveDAC1_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 WaveDAC1_DacClk_GetSourceRegister(void) 
{
    return WaveDAC1_DacClk_MOD_SRC & WaveDAC1_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: WaveDAC1_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void WaveDAC1_DacClk_SetPhaseRegister(uint8 clkPhase) 
{
    WaveDAC1_DacClk_PHASE = clkPhase & WaveDAC1_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: WaveDAC1_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 WaveDAC1_DacClk_GetPhaseRegister(void) 
{
    return WaveDAC1_DacClk_PHASE & WaveDAC1_DacClk_PHASE_MASK;
}
